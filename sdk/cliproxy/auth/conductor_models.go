...
func (m *Manager) applyAPIKeyModelAliasWithRouting(routing *apiKeyModelRoutingSnapshot, auth *Auth, requestedModel string) string {
	...
	case "cursor":
		upstreamModel = resolveUpstreamModelForCursorAPIKey(cfg, auth, requestedModel)
	case "codex":
...
func resolveCursorAPIKeyConfig(cfg *internalconfig.Config, auth *Auth) *internalconfig.CursorKey {
	if cfg == nil {
		return nil
	}
	return resolveAPIKeyConfig(cfg.CursorKey, auth)
}
...
func resolveUpstreamModelForCursorAPIKey(cfg *internalconfig.Config, auth *Auth, requestedModel string) string {
	entry := resolveCursorAPIKeyConfig(cfg, auth)
	if entry == nil {
		return ""
	}
	return resolveModelAliasFromConfigModels(requestedModel, asModelAliasEntries(entry.Models))
