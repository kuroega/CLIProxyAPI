...
func validateCredentialWeightYAML(data []byte) error {
	...
	families := map[string]struct{}{
		...
		"vertex-api-key": {}, "codex-api-key": {}, "cursor-api-key": {}, "xai-api-key": {},
	}
...
func (cfg *Config) ValidateCredentialWeights() error {
	...
	for index := range cfg.CursorKey {
		if errValidate := ValidateCredentialWeight(cfg.CursorKey[index].Weight); errValidate != nil {
			return fmt.Errorf("cursor-api-key[%d].weight: %w", index, errValidate)
		}
	}
